...
export default function RemindersScreen() {
  ...
  const renderItem = ({ item }: { item: SupplyRow }) => {
    ...
    const due = firstDueAfterLastReplaced(
      item.lastReplaced,
      item.intervalDays,
      item.notifyHour,
      item.notifyMinute
  ...
  return (
    <>
      <LinearGradient colors={gradientColors} style={styles.container}>
        {/* ✅ Stars are true background (absolute) */}
        <BackgroundStars visible={scheme === 'dark'} seed={42} />

        {/* ✅ Header with its own backdrop mask so list overflow can't show through */}
        <View style={[styles.headerWrap, { paddingTop: insets.top + 8 }]}>
          <View pointerEvents="none" style={styles.headerBackdrop}>
            <LinearGradient colors={gradientColors} style={StyleSheet.absoluteFillObject} />
            {/* Optional: render stars here too so the header backdrop matches perfectly */}
            {scheme === 'dark' && <BackgroundStars visible seed={42} />}
...
const styles = StyleSheet.create({
  ...
  // ✅ Full-width header wrapper that masks overflow from FlatList
  headerWrap: {
    marginHorizontal: -16,     // extend backdrop to edges (container has paddingHorizontal:16)
  ...
  // ✅ Keep overflow visible so glow works
